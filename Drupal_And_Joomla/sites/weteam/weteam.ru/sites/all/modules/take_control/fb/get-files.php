...
function take_control_fb_list_subdirs() {
  $node = $_REQUEST['node'];
  take_control_fb_validate_path($node);

  take_control_fb_scan_dir($node, $subdirs, $files);
...
function take_control_fb_create_folder() {
  ...
  $name = $_REQUEST['name'];
  take_control_fb_validate_path($parent);
  take_control_fb_validate_name($name);
...
function take_control_fb_copy() {
  ...
  take_control_fb_validate_path($path);
...
function take_control_fb_file_download() {
  $file = $_REQUEST['file'];
  take_control_fb_validate_path($file);
...
function take_control_fb_create_archive() {
  ...
  take_control_fb_validate_paths($nodes);
  take_control_fb_validate_path($file);
  take_control_fb_validate_path($curDir);
  ...
  foreach ($nodes as $path) {
    if (is_file($path)) {
      take_control_fb_zip_addFile($zip, $path, $curDir);
    }
    else {
      if (!is_dir($path)) {
        $zip->close();
      ...
      take_control_fb_recurse_zip($zip, $path, $curDir);
    }
  }
  $zip->close();
}
...
function take_control_fb_rename() {
  ...
  $new = $_REQUEST['name'];
  take_control_fb_validate_path($path);
  take_control_fb_validate_name($new);
...
function take_control_fb_change_perm() {
  ...
  take_control_fb_validate_paths($nodes);
  take_control_fb_validate_perm($perm);
...
// helper functions
function take_control_fb_validate_path($path) {
  if (!isset($path)) {
    take_control_security_error();
  }
...
function take_control_fb_validate_paths($paths) {
  foreach ($paths as $path) {
    take_control_fb_validate_path($path);
  }
}

function take_control_fb_validate_name($name) {
  if (strpos($name, '/') !== FALSE || strpos($name, '\\') !== FALSE) {
    take_control_security_error('Invalid name.');
  }
}

function take_control_fb_validate_perm($perm) {
  $perm = (int) $perm;
...
function take_control_fb_scan_dir($base, &$subdirs, &$files) {
  $subdirs = array();
  $files = array();
    ...
    while ($f = $d->read()) {
      ...
      if (is_dir($filename)) {
        ...
        $files[] = array(
            ...
            'size' => take_control_fb_format_bytes(filesize($filename), 2));
      }
    }

    $d->close();

    usort($subdirs, 'take_control_fb_file_comparer');
    usort($files, 'take_control_fb_file_comparer');
  }
  else {
    take_control_security_error();
  }
...
function take_control_fb_recurse_zip(&$zip, $path, $basePath) {
  //The line below ensures that empty directories also get added to the zip file.
  take_control_fb_zip_addDir($zip, $path, $basePath);
  $dir = opendir($path);
  while (($file = readdir($dir)) !== false) {
    if (($file != '.') && ($file != '..')) {
      if (is_dir($file)) {
        take_control_fb_recurse_zip($zip, $file);
      }
      else {
        take_control_fb_zip_addFile($zip, $path . DIRECTORY_SEPARATOR . $file, $basePath);
      }
    }
  }
  closedir($dir);
...
function take_control_fb_zip_addFile(&$zip, $path, $basePath) {
  $name = substr($path, strlen($basePath) + 1);
  $zip->addFile($path, $name);
}

function take_control_fb_zip_addDir(&$zip, $path, $basePath) {
  $name = substr($path, strlen($basePath) + 1);
  $zip->addEmptyDir($name);
}

function take_control_fb_file_comparer($a, $b) {
  return strcmp($a["text"], $b["text"]);
}

function take_control_fb_format_bytes($val, $digits = 3, $mode = 'SI', $bB = 'B') { //$mode == 'SI'|'IEC', $bB == 'b'|'B'
